...
download_and_install() {
  if [ -x "${BIN_DIR}/${BINARY}" ] && [ "$(${BIN_DIR}/${BINARY} version | cut -d' ' -f3)" = "${SEAWEED_VERSION}" ]; then
    ...
    curl {{.ProxyConfig}} -o "$TMP_DIR/seaweed_${SEAWEED_VERSION}_${assetFileName}" -sfL "https://mirror.ghproxy.com/github.com/seaweedfs/seaweedfs/releases/download/${SEAWEED_VERSION}/${assetFileName}"

    info "Downloading ${SEAWEED_VERSION} ${assetFileName} md5"
    curl {{.ProxyConfig}} -o "$TMP_DIR/seaweed_${SEAWEED_VERSION}_${assetFileName}.md5" -sfL "https://mirror.ghproxy.com/github.com/seaweedfs/seaweedfs/releases/download/${SEAWEED_VERSION}/${assetFileName}.md5"
